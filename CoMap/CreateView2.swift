...
struct CreateView2: View {
    ...
    @State private var showingAIView: Bool = false
    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                ToolbarView(
                    ...
                    onForgeClick: { self.showingForgeView.toggle() }, 
                    onAIClick: { self.showingAIView.toggle() },
                    showingForgeView: $showingForgeView
                )
            }
        ...
        .sheet(isPresented: $showingAIView, onDismiss: addCardsToText) {
            ChatDemo()
        }
    }
