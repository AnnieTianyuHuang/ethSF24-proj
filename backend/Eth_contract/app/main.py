...
# Import the story integration module
import story_integration

contract_hash = types.UInt160.from_string("your_contract_hash_here")
...
@app.post("/register_ip_asset/")
def register_ip_asset(location_id: int):
    db = SessionLocal()
    location = db.query(Location).filter(Location.id == location_id).first()
    if location is None:
        raise HTTPException(status_code=404, detail="Location not found")
    
    response = story_integration.register_ip_asset(
        nft_contract="your_nft_contract_address",
        token_id=str(location_id),
        ip_metadata_uri="metadata_uri_here",
        ip_metadata_hash="ip_metadata_hash_here",
        nft_metadata_hash="nft_metadata_hash_here",
        nft_metadata_uri="nft_metadata_uri_here"
    )
    if 'error' in response:
        raise HTTPException(status_code=400, detail="Failed to register IP asset")
    return {"message": "IP Asset registered", "data": response}
